...
- **visibleForEntityContext** - allows to filter the nodes that are shown based on the [entity context](./luigi-context.mdx#entity-context). Currently, the backend provides an entity context for the entities `project` and `component`. A node is shown, if the entity context provides all values as required by the filter, for this the [_.isMatch](https://lodash.com/docs/4.17.15#isMatch) function is used. The format of the filter is the following:
...
- **visibleForContext** - allows to filter the nodes that are shown based on the [context](./luigi-context.mdx). In this field, a [JMESPath](https://jmespath.org/specification.html) query can be defined. When it evaluates to `true`, the node will be shown.

- **configurationMissing** - allows to show a "configuration missing" page based on the [context](./luigi-context.mdx). In this field, a [JMESPath](https://jmespath.org/specification.html) query can be defined. When it evaluates to `true`, the "configuration missing" page will be shown, otherwise the normal page.
